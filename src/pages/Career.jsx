    ...
    <div>
      ...
      <div className="relative">
        ...
        <div className="text-center w-full h-full absolute top-0 left-0 flex flex-col justify-center items-center container-x">
          <h1 className="!text-white pt-[50px]">Shine Bright With Us</h1>
        </div>
      </div>
      {/* lISTING */}
      <div className="container-xy text-[--bg] space-y-5 lg:space-y-7">
        <h2 className="text-center">Open Positions</h2>
        <div className="max-w-[700px] xl:max-w-[50vw] mx-auto divide-y divide-gray-200 border border-gray-200 shadow-lg shadow-gray-200 rounded-lg py-1 px-4 xl:px-5 flex flex-col">
          {JOB_LIST.map((job, index) => (
            <JobPositionCard data={job} key={index} />
          ))}
        </div>
      </div>
      <div className="bg-[--bg] container-x py-10 text-center space-y-7">
